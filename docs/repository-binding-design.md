...
Status: implemented and verified on `feat/repository-binding-v2`
...
Repository binding is not required for the `0.4.1` synthetic capsule diagnostic.
That command encrypts one fixed synthetic fixture with an empty working directory
and no Git metadata; it cannot read, transfer, import, or resume a real session.
Repository binding is a release gate for real-session Cloud capsules and should
also harden direct cross-device send/receive.
...
The `0.4.1` synthetic diagnostic in
[`packages/cli/src/capsule-test.ts`](../packages/cli/src/capsule-test.ts) uses a
...
No Cloud upload path is enabled. Real session and capsule uploads remain
disabled.
...
Repository binding must not be added hastily to the verified `0.4.1` synthetic
candidate. The physical two-device synthetic test has passed, and the candidate
should remain scoped to device enrollment and synthetic envelope diagnostics.

Before any real-session Cloud push, pull, open, inbox, or resume:
...
- `packages/cli/src/main.ts` and `packages/cli/src/completion.ts` for help and
  shell interfaces;
- `packages/cli/test/repository-binding.test.ts` and command/network/capsule
  regression tests.
