...
npm `latest` is `0.5.1`, including the compacted Codex-to-Claude resume fix. See
[v0.5.1 release notes](docs/releases/v0.5.1.md) for the patch and
[v0.5.0 release notes](docs/releases/v0.5.0.md) for Cloud scope and owner-only
...
Targets are instance-aware: use qualified destinations such as
`--to claude@personal` when selecting a named harness instance. Direct
`send` and Cloud `push` remain concrete-only and accept `full`, `slim`, or
`compact`, not `auto`. Devin's foreign-history safeguard is enforced in
UTF-8 bytes, including Unicode-heavy transcripts.
