...
harness instance. The Cloud-free `0.5.0-dev.0` development path additionally
binds a sanitized source repository identity to an explicitly selected target
checkout, rejects legacy unbound payloads, and rewrites the imported session to
a target-local monorepo directory without mutating Git or workspace files. It
